...
def create_requirements_file(
    ...
    """Create a temporary requirements file with the correct modl source."""
    ...
    with open(file=temp_req.name, mode="w", encoding="utf-8") as f:
        ...
        for req in requirements:
            if not (
                req.strip().startswith("-e") or req.strip().startswith("modl")
            ):
                f.write(req)
...
def run(
    ...
    use_local: bool = typer.Option(
        True, "--local", "-l", help="Use local version of modl"
    ),
